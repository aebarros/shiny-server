...
server <- function(input, output, session) {
  ...
  #for map generation
  filtered<-reactive({
    filtered.species<- data[data$comname==input$species,]
    ...
    filtered.dates=filtered.dates%>%
      group_by_("longitude","latitude","polstn","comname")%>%
  ...
  myfun <- function(map){
    ...
    addCircleMarkers(map, data = filtered(),lng = ~longitude, lat = ~latitude,radius=~ifelse(is.na(filtered()$CPUE),2,10),
      ...
      addLegend("bottomleft", pal=pal, values=filtered()$CPUE, title="Catch per cubic meter",
                opacity=1)
  }
